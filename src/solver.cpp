...
Result solveNonlinear(const Model& model) {
  ...
  double dLambda = std::clamp(model.step.initialIncrement, model.step.minIncrement, model.step.maxIncrement);
  ...
  while (lambdaPrev < model.step.totalLoadFactor - 1e-14) {
    ...
    for (int it = 0; it < model.step.maxNewtonIters; ++it) {
      ...
      if (resNorm < model.step.tolerance) {
        ...
        if (model.step.useArcLength) {
          ...
          if (it <= 3) dLambda = std::min(dLambda * 1.2, model.step.maxIncrement);
          if (it > 10) dLambda = std::max(dLambda * 0.5, model.step.minIncrement);
        }
        break;
      }
    ...
    if (converged) {
      ...
      cutbackCount = 0;
      if (model.step.fieldOutputFrequency > 0 && (incId % model.step.fieldOutputFrequency == 0)) {
        r.displacementFrames.push_back(u);
        r.frameLoadFactors.push_back(lambdaPrev);
      }
      std::cout << "[Nonlinear] Increment " << incId << " converged.\n";
    } else {
      dLambda *= model.step.useArcLength ? model.step.arcLengthShrinkFactor : 0.5;
      dLambda = std::clamp(dLambda, model.step.minIncrement, model.step.maxIncrement);
  ...
  if (!r.displacementFrames.empty()) {
    for (const auto& uf : r.displacementFrames) {
      std::vector<double> rf(ndof, 0.0);
      for (int i = 0; i < ndof; ++i) {
        for (int j = 0; j < ndof; ++j) rf[i] += K[i][j] * uf[j];
        rf[i] -= ffinal[i];
      }
